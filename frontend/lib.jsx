...
export const CompSingleSelectionScrolledFiltered = ({ style, content_style, onSelect, options }) => {
  ...
  return (
    <div className='expand' style={style}>
      ...
      <div className='scrollWindow' style={{background:"#DEE"}}><div className='scrollContent' >
        {filtered_candidates.map(cand => {
          const selected = selected_item === cand.id;
          const onClick = onSelect && (() => { set_selected_item(cand.id); onSelect(cand.id); });
          return <button key={cand.id} style={{ backgroundColor: selected ? "#acb7ceff" : null }} onClick={onClick}>{cand.id}</button>;
        })}
      </div></div>
    </div>
  );
...
CompSingleSelectionScrolledFiltered.defaultProps = {
  options: {
    candidates: [],
    filter_options: [],
...
export const FullHorizontalButton = ({ onclick, inner_style, outer_style }) => {
